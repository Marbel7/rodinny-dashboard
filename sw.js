...
// Stabilní PWA: pouze několik deterministických oprav zdrojového HTML.
// Žádný DOM observer, žádné runtime přepisování po načtení stránky.
const CACHE_NAME = 'rodinny-dashboard-v9';
...
self.addEventListener('fetch', function(event) {
  ...
  // Firebase/Google requests musí jít přímo na síť.
  if (url.hostname.includes('firebase') ||
      url.hostname.includes('googleapis.com') ||
      url.hostname.includes('gstatic.com')) return;

  // Opravujeme pouze HTML navigaci. Ostatní assety necháváme zcela být.
  ...
  event.respondWith((async function() {
    try {
      ...
      // 1) Výchozí panel: po přihlášení se Přehled nastavuje dvakrát,
      // druhýkrát v requestAnimationFrame. Tím odstraníme závod starého DOM/PWA stavu.
      ...
      // 2) Seznamy: uložení přes merge je odolnější a zachovává ostatní pole dokumentu.
      ...
      // 3) Nákupní formulář: pouze CSS, žádné změny DOM.
...
@media(max-width:560px){#seznam-detail #sz-qty{flex-basis:92px!important;width:92px!important}#seznam-detail .card>div[style*="display:flex"]{gap:6px!important}}
      ...
      if (!html.includes('id="shopping-list-fix"')) {
